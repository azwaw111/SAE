...
int main(int argc, char *argv[]) {
    ...
    while ((caractere = fgetc(fichier_entree)) != EOF) {
        if (caractere == '=') { // ignorer '='
            putchar(caractere);
            continue;
        }
        int indice_plain = indice_base64((char)caractere);
        if (indice_plain < 0) continue;

        int indice_cle_val = indice_base64(cle[indice_cle]);
        int indice_chiffre = (indice_plain + indice_cle_val) % 64;

        putchar(ALPHABET[indice_chiffre]);
